...
StatusOr<const proto::DateTime*> GetEndOfLastEncounter(
...
template <typename BundleLike,
          typename EncounterLike = BUNDLE_TYPE(BundleLike, encounter)>
StatusOr<const proto::DateTime*> GetStartOfFirstEncounter(
    const BundleLike& bundle) {
...
const std::vector<std::pair<proto::DateTime, std::unordered_set<string>>>
...
void StampWrapAndAdd(std::vector<ContainedResourceLike>* versioned_resources,
  ...
  proto::Meta* meta = MutableMetadataFromResource(resource);
  meta->mutable_version_id()->set_value(absl::StrCat(version_id));
  proto::Instant* last_updated = meta->mutable_last_updated();
  ...
  proto::Instant::Precision precision;
  const bool parse_successful = proto::Instant::Precision_Parse(
  ...
  if (parse_successful) {
    ...
    last_updated->set_precision(proto::Instant::SECOND);
    absl::TimeZone tz;
...
void SplitResource(const R& resource, const proto::VersionConfig& config,
  ...
  for (const auto& override_map_entry : sorted_overrides) {
    ...
    proto::DateTime override_time = override_map_entry.first;
...
void SplitPatient(PatientLike patient, const BundleLike& bundle,
  ...
  if (patient.deceased().has_date_time()) {
    ...
    StatusOr<const proto::DateTime*> last_time = GetEndOfLastEncounter(bundle);
    if (last_time.ok()) {
      // Use end of last encounter as time of death, with a delay to put it
      // a safe distance in the future.
      proto::DateTime input_time(*last_time.ValueOrDie());
  ...
  if (patient.has_birth_date()) {
    ...
    StatusOr<const proto::DateTime*> first_time =
    ...
    if (first_time.ok()) {
      ...
      proto::DateTime input_time(*first_time.ValueOrDie());
      input_time.set_value_us(
