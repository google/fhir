...
public class ProtoGenerator {
  ...
  // that primitive type.  For all other types, this is a Message type name generated by
  // getProfileTypeName.
  private final ImmutableMap<String, String> knownStructureDefinitionTypes;

  // Mapping from urls for StructureDefinition that the generator is aware of, to the package they
  // are defined on.
  ...
  public DescriptorProto generateProto(StructureDefinition def) {
    ...
    String packageFromKnownTypes = knownStructureDefinitionPackages.get(def.getUrl().getValue());
    if (packageFromKnownTypes != null && !packageFromKnownTypes.equals(packageName)) {
      throw new IllegalArgumentException(
          "Inconsistent package name for "
              + def.getUrl().getValue()
              + ".  Registered in --known_types in "
              + packageFromKnownTypes
  ...
  private FieldDescriptorProto buildField(
    ...
    if (isExternalExtension(element)) {
      ...
      if (!knownStructureDefinitionTypes.containsKey(profileUrl)) {
        // We don't know about this extension.
        // Return null, indicating that we shouldn't generate a proto field for this extension.
        return null;
      }
      String fieldTypeString = knownStructureDefinitionTypes.get(profileUrl);
  ...
  private String normalizeType(ElementDefinition.TypeRef type) {
    ...
    } else if (knownStructureDefinitionTypes.containsKey(type.getProfile().getValue())) {
      return knownStructureDefinitionTypes.get(type.getProfile().getValue());
    } else {
      throw new IllegalArgumentException(
  ...
  private String getTypedReferenceName(List<ElementDefinition.TypeRef> typeList) {
    ...
    for (String r : refTypes) {
      if (!r.isEmpty()) {
        if (knownStructureDefinitionTypes.containsKey(r)) {
          r = knownStructureDefinitionTypes.get(r);
        } else {
