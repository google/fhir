...
import com.google.fhir.r5.core.Element;
import com.google.fhir.r5.core.Extension;
import com.google.fhir.r5.fhirproto.PrimitiveHasNoValue;
...
public abstract class PrimitiveWrapper<T extends Message> {
  ...
  /**
   * True if the primitive wrapped by this is has a value, as opposed to being purely defined by
  ...
  public static boolean hasValue(MessageOrBuilder message) {
    ImmutableList<PrimitiveHasNoValue> extensions =
  ...
  public <B extends Message.Builder> B copyInto(B builder) {
    ...
    if (wrappedDescriptor.getName().equals(builderDescriptor.getName())) {
      // They're not the same type, but they have the same name.  (e.g., STU3 String vs R5 String).
