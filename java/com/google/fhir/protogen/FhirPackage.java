 ...
 * PackageInfo file that describes the package (must be supplied explicitly if it is not part of the
 * ZIP being loaded). The ZIP is generated by the `fhir_package` rule in protogen.bzl.
 *
...
public class FhirPackage {
   ...
   *
   * <p>A PackageInfo text proto is expected, ending in either "package_info.prototxt" or
   * "package_info.textproto". An InvalidFhirException is thrown if one is not found.
   *
   * <p>To side load a PackageInfo proto, use the version of this that accepts a PackageInfo proto.
   *
   * <p>TODO: Support NPM-based FHIR Packages for proto generation.
   *
   * @param zipFilePath The absolute path to the ZIP file that is loaded. The ZIP is expected to, at
   *     minimum, contain package information in a file ending in
   *     `package_info.[prototxt|textproto]`.
   */
  public static FhirPackage load(String zipFilePath) throws IOException, InvalidFhirException {
    ...
    if (packageInfo == null) {
      throw new IllegalArgumentException(
          "FhirPackage does not have a package_info.prototxt: " + zipFilePath);
    }
  ...
  private static FhirPackage makeFromJsonAndPackageInfo(
    ...
    if (packageInfo.getProtoPackage().isEmpty()) {
      throw new IllegalArgumentException("PackageInfo must specify `proto_package`.");
    }
    if (packageInfo.getFhirVersion() == FhirVersion.FHIR_VERSION_UNKNOWN) {
      throw new IllegalArgumentException("PackageInfo must specify `fhir_version`.");
    }
